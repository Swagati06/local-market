  ...
  render() {
    return (
      ...
      <div style={{ width: "100%", height: "90%", margin: "auto" }}>
            ...
            <div style={{display:'flex',flex: 1, flexDirection:'row', padding: 10, flexwrap: 'wrap', justifyContent:'space-around'}}>
            {
              card_contents.map((item, index) => (<Card title={item.title} image={item.url} />))
            }
            </div>
      </div> 
    );
